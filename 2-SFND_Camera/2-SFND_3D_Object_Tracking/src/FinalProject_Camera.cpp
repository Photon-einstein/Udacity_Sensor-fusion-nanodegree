...
int main(int argc, const char *argv[])
{
    ...
    bool bLogging = true;
    std::fstream resultTTCLogging, resultTTCLidar, resultTTCCamera;
    ...
    // create a folder to do evaluation of results
    if (bLogging) {
        system("clear");
        system("rm -rf ../results");
        system("mkdir -p ../results");
        resultTTCLogging.open("../results/" + detectorType + "-" + descriptorType + "-TTC.txt", std::ios::app);
        resultTTCLidar.open("../results/" + detectorType + "-" + descriptorType + "-TTC_Lidar.txt", std::ios::app);
        resultTTCCamera.open("../results/" + detectorType + "-" + descriptorType + "-TTC_Camera.txt", std::ios::app);
    }
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    if (bVis)
                    {
                        ...
                        putText(visImg, str, cv::Point2f(80, 50), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0,0,255), thickness);
                    ...
                    if (bLogging == true) {
                        resultTTCLogging<<"Frames "<<frameNumber<<"-"<<frameNumber+1<<":\tTTC Lidar: "<<ttcLidar<<",\tTTC Camera: "<<ttcCamera<<std::endl;
                        resultTTCLidar<<ttcLidar<<" ";
                        resultTTCCamera<<ttcCamera<<" ";
                    }
                    bVis = false;
                    ++frameNumber;
                } // eof TTC computation
    ...
    if(bLogging == true) {
        resultTTCLogging.close();
        resultTTCLidar.close();
        resultTTCCamera.close();
    }

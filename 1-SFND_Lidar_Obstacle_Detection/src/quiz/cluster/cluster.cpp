...
void euclideanClusterProximity(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol, std::vector<int>& cluster, std::vector<bool>& processed, int index);
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	float minClusterSize = 100, maxClusterSize = 25000;
	std::vector<std::vector<int>> listClusters;
	std::vector<bool> processed(points.size(), false);
	int index, numberPoints = points.size();
	// use KdTree object for the search method of the extraction
	for(index = 0; index < numberPoints; ++index) {
		if(processed[index] == false) {
			std::vector<int> cluster;
			euclideanClusterProximity(points, tree, distanceTol, cluster, processed, index);
...
void euclideanClusterProximity(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol, std::vector<int>& cluster, std::vector<bool>& processed, int index)
{
    ...
    for (int id : nearbyPoints)
    {
        if (!processed[id])
        {
            euclideanClusterProximity(points, tree, distanceTol, cluster, processed, id);
        }
    }
} 
